...
def _cmd_sample(args: argparse.Namespace) -> int:
    ...
    # evening and cannot be regenerated by anything. The README's usage example wrote to
    # `review/batch.md` -- a real filled sheet, since renamed `shape-A-batch-03.md` -- so
    # following the README as printed destroyed 25 verdicts, and at the time that file was
    # untracked, so there was no git copy either. Checked before the sampling so a refusal
    # costs nothing.
    if args.out.exists() and not args.force:
        print(
            f"refusing to overwrite existing {args.out}\n"

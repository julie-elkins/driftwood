...
def _cmd_sample(args: argparse.Namespace) -> int:
    ...
    # Same guard as `mine`, and this one protects strictly more. `mine` writes derived
    # data and the file it was clobbering was recoverable from git; a review sheet is
    # HAND LABELS, so a filled one is the only copy of a judgement that took a person an
    # evening and cannot be regenerated by anything. The README's usage example writes to
    # `review/batch.md`, which is a real filled sheet, so following the README as printed
    # destroyed 25 verdicts -- and at the time that file was untracked, so there was no
    # git copy either. Checked before the sampling so a refusal costs nothing.
    if args.out.exists() and not args.force:
        print(
            f"refusing to overwrite existing {args.out}\n"
            "  a review sheet may hold hand labels, which nothing regenerates.\n"
            "  choose a new --out (e.g. review/shape-A-batch-04.md), or pass --force.",
            file=sys.stderr,
        )
        return 2
...
def main(argv: list[str] | None = None) -> int:
    ...
    sample.add_argument(
        "--force",
        action="store_true",
        help="overwrite an existing --out. A filled sheet is hand labels and nothing "
        "regenerates them, so this is not the same favour it is on `mine`.",
    )
